...
class _SurahAyahSelectors extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final totalAyahs = surahId != null
        ? (ref.watch(surahByIdProvider(SurahIdKey(surahId!))).maybeWhen(
            data: (s) => s?.ayahCount ?? 0,
            orElse: () => 0,
          ))
        : 0;
        ...
        children: [
          Expanded(
            child: _SelectorTile(
              ...
              label: 'Сура',
              value: surahId?.toString() ?? 'Выбрать',
              subtitle: playerState.surahName ?? (surahId?.toString() ?? ''),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                mainAxisSize: MainAxisSize.min,
                children: [
                  // Полное имя чтеца — до 2 строк. Раньше
                  // `maxLines: 1, overflow: ellipsis` обрезало
                  // «Мишари Рашид Аль-Афаси» до «Мишари Аль Иф…» на
                  // узкой панели.
                  Text(
                    display,
                    maxLines: 2,
                  ...
                  // 128 kbps бейдж + rewaya в одной строке под именем.
                  const SizedBox(height: 4),
                  Row(
                    children: [
                      // 128 kbps бейдж — показываем всегда (mp3quran.net
                      // отдаёт 128 kbps на всех серверах, см. AGENTS.md).
                      Container(
                        padding: const EdgeInsets.symmetric(
                            horizontal: 6, vertical: 2),
                        decoration: BoxDecoration(
                          color: _kGoldDeep.withValues(alpha: 0.4),
                      ...
                      if (subtitle.isNotEmpty && subtitle != display) ...[
                        const SizedBox(width: 8),
                        Expanded(
                          child: Text(
                            subtitle,
            ...
            if (selected)
              const Icon(Icons.check, color: _kGold, size: 20),
          ],
        ),
      ),
    );
